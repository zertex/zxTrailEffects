...
@CommandDescription("Show smoke trail")
@CommandPermissions("zxtraileffects.smoke")
...
public class SmokeCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 1 && args[0].equalsIgnoreCase("smoke")) {
            Functions.removePlayerEffect(player);
            Functions.addPlayerEffect(player, "smoke");
            player.sendMessage(new Messaging.MessageFormatter().format("trails.smoke"));
        }
        else {
            player.sendMessage("Use: /trail smoke");
        }
        return true;
    }
}
