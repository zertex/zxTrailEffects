...
@CommandDescription("Show letters trail")
@CommandPermissions("zxtraileffects.letter")
...
public class LettersCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 1 && args[0].equalsIgnoreCase("letter")) {
            Functions.removePlayerEffect(player);
            Functions.addPlayerEffect(player, "letter");
            player.sendMessage(new Messaging.MessageFormatter().format("trails.letter"));
        }
        else {
            player.sendMessage("Use: /trail letter");
        }
        return true;
    }
}
