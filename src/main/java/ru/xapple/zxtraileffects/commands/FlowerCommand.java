...
@CommandDescription("Show flowers trail")
@CommandPermissions("zxtraileffects.flower")
...
public class FlowerCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 1 && args[0].equalsIgnoreCase("flower")) {
            Functions.removePlayerEffect(player);
            Functions.addPlayerEffect(player, "flower");
            player.sendMessage(new Messaging.MessageFormatter().format("trails.flower"));
        }
        else {
            player.sendMessage("Use: /trail flower");
        }
        return true;
    }
}
