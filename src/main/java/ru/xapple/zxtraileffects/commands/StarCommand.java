...
@CommandDescription("Show stars trail")
@CommandPermissions("zxtraileffects.star")
...
public class StarCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 1 && args[0].equalsIgnoreCase("star")) {
            Functions.removePlayerEffect(player);
            Functions.addPlayerEffect(player, "star");
            player.sendMessage(new Messaging.MessageFormatter().format("trails.star"));
        }
        else {
            player.sendMessage("Use: /trail star");
        }
        return true;
    }
}
